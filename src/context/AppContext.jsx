...
const DEFAULT_PREFERENCES = [
  { name: "Academic", preferredTime: "morning", intensity: "high" },
  { name: "Work", preferredTime: "afternoon", intensity: "medium" },
  { name: "Health", preferredTime: "morning", intensity: "medium" },
  { name: "Leisure", preferredTime: "evening", intensity: "low" },
];
...
export function AppProvider({ children }) {
  ...
  const updateDeadline = async (updatedDeadline) => {
    try {
  ...
  const getUpcomingTasks = () => {
    ...
    return tasks
      ...
      .map(task => ({
        ...task,
        _dateTime: buildDateTime(task.date, task.time),
      }))
      .filter(t => t._dateTime && t._dateTime >= now)
  ...
  const getReminderTasks = () => {
    ...
    return reminders
      ...
      .filter(r => r.task && r.reminderDateTime && r.reminderDateTime >= now)
  ...
  useEffect(() => {
    try {
      ...
      setSchedulePreferences(DEFAULT_PREFERENCES);
    }
  }, []);
